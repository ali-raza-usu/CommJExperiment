...
import exp.ftp.messages.FileTransferAck;
...
public class FTPServer extends Thread {
	...
	Protocol _protocol = new Protocol(UUID.randomUUID());
		...
		try {
			...
			try {
				...
				while (!transferComplete) {
					// _logger.debug("Goin in the sleelct");
					sckt_manager.select();
					// _logger.debug("out of the sleelct");
					for (Iterator<SelectionKey> i = sckt_manager.selectedKeys()
						...
						if (key.isReadable()) {
							buffer = ByteBuffer.allocateDirect(5024);// this is
																		// useless
																		// line
							buffer.clear();
							client.read(buffer);
								...
								if (_fileTransferRequest.getFileIndex() == null) {
									...
									if (selectedFileIndx != null) {
										...
										while (!isFinnished) {
											...
											// _logger.debug("finnsihed reading from file ");
											FileTransferResponse _resp = null;
											if (remainingBytes == 0) {
												// _logger.debug("Finnished sending the file to the client with bytes "+
												// rdBytes.length);
												...
												// _logger.debug("seding NOT final chunk");
												_resp = new FileTransferResponse(
														selectedFile.getName(),
														rdBytes, false);
											}
